...
class Account(models.Model):
    ...
    def __unicode__(self):
        return '%s %s %s %s %s %s %s %s %s' % (self.id, self.bank_id, self.partner_id, self.account_name, self.account_number, self.status, self.deleted, self.created_at, self.updated_at)
...
class AccountMeta(models.Model):
    ...
    account_id = models.ForeignKey(
        Account, default=uuid.uuid4, max_length=36, on_delete=models.DO_NOTHING, db_column='account_id')
    ...
    def __unicode__(self):
        return '%s %s %s %s %s %s %s' % (self.id, self.account_id, self.key, self.value, self.deleted, self.created_at, self.updated_at)
...
class AccountTransaction(models.Model):
    ...
    def __unicode__(self):
        return '%s %s %s %s %s %s %s %s %s %s %s' % (self.id, self.account_id, self.post_date, self.debit, self.credit, self.balance, self.note, self.status, self.deleted, self.created_at, self.updated_at)
...
class TransferRequest(models.Model):
    ...
    transfer_amount_id = models.ForeignKey(
        TransferAmount, default=uuid.uuid4, max_length=36, on_delete=models.DO_NOTHING, db_column='transfer_amount_id')
    status = models.TextField(max_length=50, blank=False, null=False)
...
class SystemConfig(models.Model):
    ...
    def __unicode__(self):
        return '%s %s %s %s %s %s' % (self.id, self.key, self.value, self.deleted, self.created_at, self.updated_at)
...
class SystemApp(models.Model):
    ...
    def __unicode__(self):
        return '%s %s %s %s %s %s %s %s %s' % (self.id, self.name, self.api_key, self.type, self.status, self.ip, self.deleted, self.created_at, self.updated_at)
